...
**Action items**:
- [x] Add `facet` and `facet_ncol` parameters to `run_from_json()` in histogram_template.py
- [x] Pass only user-facing facet controls through template into `histogram()`
- [x] Pass `target_fig_width` and `target_fig_height` from template into `histogram()` as internal kwargs
- [x] Remove `facet_vertical_threshold`, `facet_height`, and `facet_aspect` from histogram docstring/API contract
...
**Action items**:
- [x] Define minimum safe panel size/aspect guardrails (for dense facet grids)
- [x] Compute derived facet geometry when `facet=True` using template-provided `target_fig_width`/`target_fig_height` + `facet_ncol`
...
Status: **De-scoped for current histogram-facet PR**. Keep tracked as future UI integration follow-up.
...
**Action items**:
- [ ] Revisit axis abbreviation when preparing SPAC_Shiny integration PR
...
- [x] Add `facet_ncol` to `histogram_template.py` as the only user-facing facet layout control
- [x] Add `facet` to `histogram_template.py` as user-facing behavior switch
- [x] Pass `target_fig_width`/`target_fig_height` into `histogram()` as internal sizing hints
- [x] Remove `facet_vertical_threshold`/`facet_height`/`facet_aspect` from histogram docstring and accepted public controls
- [x] Expose `element` in template and pass through to `histogram()`
- [x] Add template-side validation for `facet_ncol` and enum-like plotting options (`multiple`, `element`, `stat`)
- [ ] Document sizing precedence in template docstring: `Figure_Width`/`Figure_Height` are authoritative
- [x] Add derived geometry step: compute internal `facet_height`/`facet_aspect` from figure size + facet grid shape
- [x] Define minimum panel-size/aspect guardrails
- [x] Add safeguard to prevent facet-only kwargs from leaking into non-facet seaborn paths
...
- [ ] Decide bins policy for `"auto"`/`None` in non-facet grouped paths (double-binning issue) with mentor
- [ ] Align histogram bins docstrings with decided bins policy
- [ ] Add focused tests for bins behavior after policy decision
